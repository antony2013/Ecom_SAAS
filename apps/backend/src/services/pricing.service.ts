...
export const pricingService = {
   ...
   * Looks up product, variant options, combination, and modifier adjustments from the DB.
   */
  async computeItemPrice(params: ComputeItemPriceParams): Promise<ComputedItemPrice> {
    ...
    if (variantOptionIds && variantOptionIds.length > 0) {
      const options = await db.query.productVariantOptions.findMany({
        where: and(
          inArray(productVariantOptions.id, variantOptionIds),
      ...
      const variants = await db.query.productVariants.findMany({
        where: and(
          inArray(productVariants.id, optionVariantIds),
    ...
    if (combinationKey) {
      const combination = await db.query.productVariantCombinations.findFirst({
        where: and(
    ...
    if (modifierOptionIds && modifierOptionIds.length > 0) {
      const modifierOpts = await db.query.modifierOptions.findMany({
        where: and(
          inArray(modifierOptions.id, modifierOptionIds),
  ...
  async computeOrderPricing(params: ComputeOrderPricingParams): Promise<ComputedOrderPricing> {
    ...
    if (shippingRateId && shippingAddress.country) {
      ...
      if (shippingResult.options.length > 0) {
        const selectedOption = shippingResult.options.find((opt) => opt.id === shippingRateId);
